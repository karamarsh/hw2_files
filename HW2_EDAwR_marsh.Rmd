...
str(county_MI)
# The Michigan-only data has a column for peninsula, and the County, State,  CensusId, State.Code, Region, and Division columns are in a different order

sum(is.na(county_df)) # Yes, there are three NA values in this data set
sum(is.na(county_MI)) # No, there are no NA values in this data set
...
Nearly all counties have a median income between 25,000 and 75,000, though there are a few dozen outliers with larger median incomes up to 125,000. 
...
g + geom_histogram(aes(x = TotalPop), fill = "Blue") + labs(title = "Distribution of population for US counties", x ="Total Population", y = "Number of counties")
```
...
```{r histo_lt2.5m}
g + geom_histogram(aes(x = TotalPop), fill = "Blue") + labs(title = "Distribution of population for US counties", x ="Total Population", y = "Number of counties") + geom_density(aes(x = TotalPop), fill = "grey50")

smaller <- county_df %>%
  filter(TotalPop < 2500000)

ggplot(data = smaller, aes(x = TotalPop)) + geom_histogram(fill = "blue") + labs(title = "Distribution of non-outlier population for US counties", x ="Total Population", y = "Number of counties") + geom_density()
```
...
ggplot(data = county_MI) + geom_boxplot(aes(y = Income, x = peninsula)) + 
  labs(title = "Distribution of income by peninsula", y = "Income", x = "Peninsula")
...
```{r more_boxplots}
ggplot(data = county_MI) + geom_boxplot(aes(y = Black, x = peninsula)) + 
  labs(title = "Percent of population that is black by peninsula", y = "Black %", x = "Peninsula")

ggplot(data = county_MI) + geom_boxplot(aes(y = IncomePerCap, x = peninsula)) + 
  labs(title = "Distribution of income per capita by peninsula", y = "Income Per Person", x = "Peninsula")

ggplot(data = county_MI) + geom_boxplot(aes(y = Poverty, x = peninsula)) + 
  labs(title = "Percent of population in poverty by peninsula", y = "% in Poverty", x = "Peninsula")

ggplot(data = county_MI) + geom_boxplot(aes(y = ChildPoverty, x = peninsula)) + 
  labs(title = "Percent of children in poverty by peninsula", y = "% of Children in Poverty", x = "Peninsula")

ggplot(data = county_MI) + geom_boxplot(aes(y = Production, x = peninsula)) + 
  labs(title = "Production Worker percentage by peninsula", y = "Production %", x = "Peninsula")

ggplot(data = county_MI) + geom_boxplot(aes(y = MeanCommute, x = peninsula)) + 
  labs(title = "Distribution of mean commute by peninsula", y = "Commute time (mins)", x = "Peninsula")
```
"Percent of population that is black by peninsula"
"Distribution of income per capita by peninsula"
"Percent of population in poverty by peninsula"
"Percent of children in poverty by peninsula"
"Production Worker percentage by peninsula"
"Distribution of mean commute by peninsula"
...
ggplot(data = county_df) + 
  geom_histogram( aes(x = WomenRatio, y = ..density..), bins = 20) + 
  geom_density(aes(x = WomenRatio)) +
  facet_wrap( ~ Region) + labs(title = "Distribution of women to men ration by region", 
...
ggplot(data = county_df) + geom_bar(aes(x = Region)) + labs(title = "Number of Counties by Region", y = "Number of Counties", x = "Region")
```
...
ggplot(data = county_MI) + geom_point(aes(x = Income, y = MeanCommute, color = peninsula))
```
